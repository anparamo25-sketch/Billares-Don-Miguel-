...
if old_collect not in source:
    raise SystemExit('ERROR: no se encontró collect() esperado')
...
if old_grid not in source:
    raise SystemExit('ERROR: no se encontró altura de tarjetas')
...
if subprocess.run(['git', 'diff', '--cached', '--quiet']).returncode != 0:
    subprocess.run(['git', 'commit', '-m', 'feat: nueva interfaz TV y cobro con recibo automatico [skip ci]'], check=True)
    subprocess.run(['git', 'push', 'origin', 'HEAD:main'], check=True)

print('OK: interfaz TV, logo, tarjetas, Cobrar e impresión automática preparados')
