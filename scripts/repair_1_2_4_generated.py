...
def mask_strings_and_comments(source: str) -> str:
    ...
    out = list(source); i = 0; n = len(source); quote = None; triple = False
    while i < n:
        if quote:
            token = quote * 3 if triple else quote
            if source.startswith(token, i):
                for j in range(i, min(i + len(token), n)): out[j] = ' '
                i += len(token); quote = None; triple = False; continue
            out[i] = '\n' if source[i] == '\n' else ' '
        ...
        if source.startswith('/*', i):
            out[i] = out[i + 1] = ' '; i += 2
            while i < n:
                if source.startswith('*/', i): out[i] = out[i + 1] = ' '; i += 2; break
                out[i] = '\n' if source[i] == '\n' else ' '; i += 1
            continue
        if source.startswith("'''", i): quote, triple = "'", True; out[i:i + 3] = [' ', ' ', ' ']; i += 3; continue
        if source.startswith('"""', i): quote, triple = '"', True; out[i:i + 3] = [' ', ' ', ' ']; i += 3; continue
        if source[i] in ("'", '"'): quote, triple = source[i], False; out[i] = ' '; i += 1; continue
        i += 1
    return ''.join(out)
...
if not re.search(r'\bMap<String,\s*dynamic>\s+stateMap\s*\(\)', current): raise SystemExit('REPAIR PREFLIGHT FAILED: stateMap del Dashboard ausente')
if not re.search(r'\bFuture\s*<\s*void\s*>\s+showTvConnection\s*\(', current): raise SystemExit('REPAIR PREFLIGHT FAILED: conexión TV ausente')
...
if not binds or not any(arg.strip() == '80' for arg in binds): raise SystemExit(f'REPAIR TV FAILED: HttpServer.bind no está correctamente configurado en puerto 80: {binds}')
...
show_tv_declarations = re.findall(r'\bFuture\s*<\s*void\s*>\s+showTvConnection\s*\(\s*\)\s+async\b', code_only)
if len(show_tv_declarations) != 1: raise SystemExit(f'REPAIR TV FAILED: showTvConnection quedó {len(show_tv_declarations)} declaraciones reales')

tv_getter_matches = list(re.finditer(r'(?m)^\s*String\s+get\s+tvHtml\s*=>\s*', code_only))
if len(tv_getter_matches) != 1: raise SystemExit(f'REPAIR TV FAILED: tvHtml quedó {len(tv_getter_matches)} veces')
start = tv_getter_matches[0].start(); end = current.find('\n', start)
...
for line in code_only.splitlines():
    if '===' in line: raise SystemExit('REPAIR TV FAILED: JavaScript quedó fuera del getter TV')
