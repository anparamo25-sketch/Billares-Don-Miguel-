...
current=TARGET.read_text();baseline=subprocess.check_output(['git','show',f'{STABLE}:lib/main.dart'],text=True)
...
if not re.search(r'\bMap\s*<\s*String\s*,\s*dynamic\s*>\s+stateMap\s*\(\s*\)',TARGET.read_text()):raise SystemExit('REPAIR PREFLIGHT FAILED: stateMap ausente')
