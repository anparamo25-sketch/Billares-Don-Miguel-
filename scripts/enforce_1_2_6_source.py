...
# These are deterministic source transformations used by the release build.
# They replace the old dashboard UI and the malformed mDNS responder before the
# final contract/build steps, then the resulting source is persisted to main.
...
# The new administrator screen is structural, not merely a marker/string.
for marker in ('_summaryCard(', 'NavigationDestination(', 'GridView.builder(', 'LAN conectado', 'Pantalla exclusiva para TV'):
    if marker not in source:
        raise SystemExit(f'1.2.6 SOURCE FAILED: falta interfaz administrativa real: {marker}')

# mDNS must use a valid DNS header: flags=0x8400, QD=0, AN=1, NS=0, AR=0.
...
print('OK: fuente final 1.2.6 con interfaz nueva y mDNS corregido')
