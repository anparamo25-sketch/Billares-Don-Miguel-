...
# Rebuild the TV getter deterministically after every historical repair.
# The historical source can be malformed before Dart parses it, so do not
# depend on Dart braces or triple-quoted strings being structurally valid.
start = s.find('  String get tvHtml')
if start < 0:
    raise SystemExit('TV FINAL FAILED: no se encontró tvHtml')
marker = '  Map<String, dynamic> stateMap()'
end = s.find(marker, start)
if end < 0:
    raise SystemExit('TV FINAL FAILED: no se encontró stateMap después de tvHtml')

# Keep the existing HTML if it can be safely extracted; otherwise use a
# compact deterministic receiver. Either way it is JSON-escaped into Dart.
region = s[start:end]
html = ''
for q in ('"""', "'''", '"', "'"):
    token = 'return r' + q
