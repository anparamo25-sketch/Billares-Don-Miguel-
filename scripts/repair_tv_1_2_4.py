...
s = TARGET.read_text()
# Canonical producer: eliminate every previous TV connection definition before
# creating exactly one. This is intentionally independent of generated class names
# and indentation because prepare_1_2_4.py regenerates the source before this step.
...
# The receiver HTML is deliberately emitted as one JSON-escaped Dart string.
...
# Producer-level invariants: exactly one connection function and one TV getter.
if len(re.findall(r'\bFuture\s*<\s*void\s*>\s+showTvConnection\s*\(\s*\)\s+async\s*\{', s)) != 1:
