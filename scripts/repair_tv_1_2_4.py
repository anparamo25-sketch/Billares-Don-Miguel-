...
def remove_definitions(source: str) -> str:
    patterns = [
        ...
        re.compile(r'(?m)^\s*String\s+get\s+tvHtml(?:Legacy\d+)?\s*=>[^;]*;\s*'),
    ]
    while True:
        matches = [m for p in patterns if (m := p.search(source))]
        if not matches:
            return source
...
s = remove_definitions(TARGET.read_text())
...
# IMPORTANT: generate the HTML as a normal Dart string literal with JSON escaping.
# This completely eliminates triple-quoted Dart strings and therefore prevents
# JavaScript operators such as === from ever escaping into Dart source.
...
# json.dumps returns a valid double-quoted Dart-compatible string literal.
...
if len(re.findall(r'(?m)^\s*Future<void>\s+showTvConnection\s*\(\)\s+async\s*\{', s)) != 1:
...
TARGET.write_text(s)
print('OK: receptor TV reconstruido como string JSON-escapado, sin triple comillas ni duplicados')
