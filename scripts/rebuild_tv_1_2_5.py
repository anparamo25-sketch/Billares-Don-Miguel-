...
def brace_end(s, start):
    ...
    while i < len(s):
        ...
        if s.startswith('/*', i):
            e = s.find('*/', i + 2)
            if e < 0: raise SystemExit('TV REBUILD FAILED: comentario sin cerrar')
            i = e + 2; continue
        if s[i] == '{': depth += 1
        elif s[i] == '}':
            depth -= 1
