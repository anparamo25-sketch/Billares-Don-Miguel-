...
mdns="""\nRawDatagramSocket? _billaresMdnsSocket;\nFuture<String?> _billaresLocalIp() async {\n  try { final interfaces=await NetworkInterface.list(type:InternetAddressType.IPv4,includeLoopback:false); for(final n in interfaces){ for(final a in n.addresses){ final p=a.address.split('.').map(int.tryParse).whereType<int>().toList(); final privateIpv4=p.length==4&&(p[0]==10||(p[0]==172&&p[1]>=16&&p[1]<=31)||(p[0]==192&&p[1]==168)); if(privateIpv4&&!a.isLoopback&&!a.isLinkLocal&&!a.isMulticast)return a.address; } } } catch(_){} return null;\n}\nFuture<void> _answerBillaresMdns(RawDatagramSocket socket,Datagram datagram) async {\n  try { final q=datagram.data;if(q.length<12)return;final questions=(q[4]<<8)|q[5];var off=12;var match=false;for(var i=0;i<questions;i++){final labels=<String>[];while(off<q.length){final n=q[off++];if(n==0)break;if(n>63||off+n>q.length)return;labels.add(String.fromCharCodes(q.sublist(off,off+n)));off+=n;}if(off+4>q.length)return;final type=(q[off]<<8)|q[off+1];off+=4;if(labels.join('.').toLowerCase()=='billaresdonmiguel.local'&&(type==1||type==255))match=true;}if(!match)return;final ip=await _billaresLocalIp();if(ip==null)return;final oct=ip.split('.').map(int.parse).toList();final r=<int>[];r.addAll(q.sublist(0,2));r.addAll(<int>[0x84,0,0,0,0,1,0,0,0,120,0,4]);r.addAll(q.sublist(12,off));r.addAll(<int>[0xC0,0x0C,0,1,0,1,0,0,0,120,0,4]);r.addAll(oct);socket.send(r,datagram.address,datagram.port);}catch(_){}\n}\nFuture<void> _startBillaresMdns() async {\n  try { _billaresMdnsSocket?.close();final socket=await RawDatagramSocket.bind(InternetAddress.anyIPv4,5353,reuseAddress:true,reusePort:true);_billaresMdnsSocket=socket;socket.joinMulticast(InternetAddress('224.0.0.251'));socket.listen((e){if(e!=RawSocketEvent.read)return;final d=socket.receive();if(d!=null)_answerBillaresMdns(socket,d);}); }catch(_){}\n}\n"""
