...
def remove_getter(s, name):
    ...
    while True:
        ...
        if not match:
            return s
        end = (brace_end(s, s.find('{', match.start(), match.end()))
               if '{' in match.group(0) else statement_end(s, match.end()))
...
for imp in ("import 'dart:io';", "import 'package:flutter/services.dart';"):
...
Future<void> _answerBillaresMdns(RawDatagramSocket socket, Datagram datagram) async {
  try {
    final q = datagram.data;
    if (q.length < 12) return;
    final questions = (q[4] << 8) | q[5];
    var offset = 12;
    var match = false;
    for (var i = 0; i < questions; i++) {
      final labels = <String>[];
      while (offset < q.length) {
        final length = q[offset++];
        if (length == 0) break;
        if (length > 63 || offset + length > q.length) return;
        labels.add(String.fromCharCodes(q.sublist(offset, offset + length)));
        offset += length;
      }
      if (offset + 4 > q.length) return;
      final type = (q[offset] << 8) | q[offset + 1];
      offset += 4;
...
if not first_class:
    raise SystemExit('TV REBUILD FAILED: no hay clases Dart')
