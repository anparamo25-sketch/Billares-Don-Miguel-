...
if "const String appVersion = '1.3.0+130';" in s:
    ...
    raise SystemExit('No se encontró una versión base compatible en lib/main.dart.')
...
summary_end = s.index(end_marker, summary_start)
summary = s[summary_start:summary_end]

def compact_print_block(block: str, label: str) -> str:
    generator_old = 'Generator(PaperSize.mm58, profile)'
    generator_new = 'Generator(PaperSize.mm58, profile, spaceBetweenRows: 0)'
    generator_count = block.count(generator_old)
    if generator_count == 0:
        if generator_new not in block:
            raise SystemExit(f'{label}: no se encontró el Generator de 58mm esperado.')
    else:
        block = block.replace(generator_old, generator_new)
...
if 'version: 1.3.0+130' in pubspec:
    ...
    raise SystemExit('No se encontró una versión base compatible en pubspec.yaml.')
pubspec_path.write_text(pubspec, encoding='utf-8')

update = update_path.read_text(encoding='utf-8')
if '"version":"1.3.0+130"' in update:
    update = update.replace('"version":"1.3.0+130"', '"version":"1.3.1+131"', 1)
elif '"version":"1.3.1+131"' not in update:
    raise SystemExit('No se encontró una versión base compatible en update.json.')
