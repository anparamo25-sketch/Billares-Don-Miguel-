...
if PUBLIC_TV_URL != 'https://billaresdonmiguel.pages.dev':
    raise SystemExit('1.3.1 CONTRACT FAILED: dominio publico de TV incorrecto')
...
for name, marker in REQUIRED_APP.items():
    if marker not in APP_SOURCE and marker not in APP_NORMALIZED:
        raise SystemExit(f'1.3.1 CONTRACT FAILED: falta {name}: {marker}')
...
if not rate_decl:
    raise SystemExit('1.3.1 CONTRACT FAILED: tarifas fijas incorrectas o ausentes')
...
for port in (80, 8080):
    ...
    if not re.search(pattern, APP_NORMALIZED):
        raise SystemExit(f'1.3.1 CONTRACT FAILED: servidor LAN sin soporte para puerto {port}')
...
for pattern, name in (
    ...
    if len(re.findall(pattern, APP_SOURCE)) != 1:
        raise SystemExit(f'1.3.1 CONTRACT FAILED: {name} debe existir exactamente una vez')
...
for name, marker in REQUIRED_TV.items():
    if marker not in TV_SOURCE and marker not in TV_NORMALIZED:
        raise SystemExit(f'1.3.1 CONTRACT FAILED: falta {name} en la fuente TV: {marker}')

for marker in ('color:#1557c0', 'font-size:clamp('):
    if marker not in TV_SOURCE:
        raise SystemExit(f'1.3.1 CONTRACT FAILED: estilo TV ausente: {marker}')
for marker in (
    '.green{',
    '.red{',
    '.yellow{',
    ...
    if marker not in TV_SOURCE:
        raise SystemExit(f'1.3.1 CONTRACT FAILED: estado visual ausente: {marker}')

if 'Finalización:' not in TV_SOURCE:
    raise SystemExit('1.3.1 CONTRACT FAILED: falta hora de finalización en TV')
if 'panel externo' in APP_SOURCE.lower() or 'panel externo' in TV_SOURCE.lower():
    raise SystemExit('1.3.1 CONTRACT FAILED: panel externo no permitido')
for forbidden in ('faltante', 'sobrante', 'diferencia de caja', 'caja cuadrada'):
    if forbidden in APP_SOURCE.lower() or forbidden in TV_SOURCE.lower():
        raise SystemExit(f'1.3.1 CONTRACT FAILED: lógica prohibida: {forbidden}')

version_match = re.search(r"const String appVersion = '([^']+)';", APP_SOURCE)
if not version_match or version_match.group(1) != '1.3.1+131':
    raise SystemExit('1.3.1 CONTRACT FAILED: la fuente no corresponde a la versión 1.3.1+131')

print(f'OK: contrato funcional 1.3.1/131 validado; TV publica: {PUBLIC_TV_URL}')
