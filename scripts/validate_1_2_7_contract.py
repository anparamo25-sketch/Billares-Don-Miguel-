...
if PUBLIC_TV_URL != 'https://billaresdonmiguel.pages.dev':
    raise SystemExit('1.3.2 CONTRACT FAILED: dominio publico de TV incorrecto')
...
for name, marker in REQUIRED_APP.items():
    if marker not in APP_SOURCE and marker not in APP_NORMALIZED:
        raise SystemExit(f'1.3.2 CONTRACT FAILED: falta {name}: {marker}')
...
if not rate_decl:
    raise SystemExit('1.3.2 CONTRACT FAILED: tarifas fijas incorrectas o ausentes')
...
for port in (80, 8080):
    ...
    if not re.search(pattern, APP_NORMALIZED):
        raise SystemExit(f'1.3.2 CONTRACT FAILED: servidor LAN sin soporte para puerto {port}')
...
for pattern, name in (
    ...
    if len(re.findall(pattern, APP_SOURCE)) != 1:
        raise SystemExit(f'1.3.2 CONTRACT FAILED: {name} debe existir exactamente una vez')
...
for name, marker in REQUIRED_TV.items():
    if marker not in TV_SOURCE and marker not in TV_NORMALIZED:
        raise SystemExit(f'1.3.2 CONTRACT FAILED: falta {name} en la fuente TV: {marker}')

for marker in ('color:#1557c0', 'font-size:clamp('):
    if marker not in TV_SOURCE:
        raise SystemExit(f'1.3.2 CONTRACT FAILED: estilo TV ausente: {marker}')
for marker in (
    '.green{',
    '.red{',
    '.yellow{',
    ...
    if marker not in TV_SOURCE:
        raise SystemExit(f'1.3.2 CONTRACT FAILED: estado visual ausente: {marker}')

if 'Finalización:' not in TV_SOURCE:
    raise SystemExit('1.3.2 CONTRACT FAILED: falta hora de finalización en TV')
if 'panel externo' in APP_SOURCE.lower() or 'panel externo' in TV_SOURCE.lower():
    raise SystemExit('1.3.2 CONTRACT FAILED: panel externo no permitido')
